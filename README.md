...
- **语境翻译**：告诉模型「这是牙膏盒包装」还是「这是 App 按钮」，选词、句式、行业标示语都会跟着变
...
4. **（可选，推荐）填语境**：在「语境」区块选场景预设或点「分析画布」，详见 3.5 节
5. 点 **扫描** → 列表里会显示每个文本对象和它的原文
6. 点 **翻译** → 译文直接写回画布，列表中原文/译文对照
7. 不满意点 **撤销**（可回滚最近 30 次操作）
...
同一个词在不同场景该翻成不同东西。`Net wt.` 在牙膏盒上是「净含量」，在工业铭牌上也可能是「净重」；`Save` 在软件菜单里是「保存」，在包装上可能是「节省」。

默认情况下每条文本是**孤立**翻译的，模型不知道自己在翻牙膏盒还是 App 按钮。**语境**就是把这个信息告诉它。

#### 怎么用

面板中部的**「语境」区块**：

1. **选场景**：下拉里有 10 个领域预设（**★ 日化 / 口腔护理（九叶本厂）** / 产品包装 / 标签铭牌 / 软件界面 / 服装吊牌 / 广告营销 / 游戏本地化 / 医疗合规 / 工业工程 / 合同法律），选中后会把一段场景说明填进下面的输入框，**你可以随意改写**。
2. **或者点「分析画布」**：把画布上的文字交给大模型，让它自己判断「这是什么产品、该用什么语气」，结果自动填进语境框，顺带给出 5–15 条关键术语并并入术语表。取样上限 120 条 / 3000 字，不会把整份文档塞过去。
3. **点翻译**：语境作为指令注入提示词，同时**同一文本框的其他段落会作为上下文一起送给模型**——它看到 `Ice Mint Toothpaste / Refreshing mint / Keep out of reach of children` 这一整版文案，才知道这是在翻牙膏盒。

> **本厂专用预设（`jiuye`）**：为日化 / 口腔护理工厂定制的语境，内置
> 牙膏 = toothpaste、漱口水 = mouthwash、活性炭 = activated charcoal、防蛀 = anticavity、
> 净含量 = Net Wt. 等 24 条行业术语基准，写明「不属于食品、餐饮、肉类、药品」，
> 并列出必须原样保留的品牌名（YATAI / SALAMANKA / DOBAOLE …）。
> 产品线、术语、品牌名都可以在填进输入框后自行改写。

以牙膏盒为例，模型实际收到的指令是（节选）：

```
你是专业的平面设计文案翻译。把下面 JSON 数组中的每一条文本翻译成 英语。
【语境】这是产品包装（盒、瓶、袋、管）上的文案，面向终端消费者。行业范围：日用化学品、
个人护理、化妆品、口腔护理、家居清洁——不属于食品、餐饮、肉类、药品或医疗器械。要求：…
翻译必须贴合上述语境：选词、句式、标示语都要符合该场景在目标语言里的惯例。
【同一版面的其他文案（仅用于理解语境，不要翻译，也不要出现在输出里）】
Ice Mint Toothpaste / Net wt. 120g / Refreshing mint, long-lasting fresh breath /
Keep out of reach of children / 120
【硬性约束（违反任意一条即为不合格）】
...
- 每条都要完整译出：除品牌名、商标、型号、条码、认证编号外，不允许原样保留原文，
  不允许截断、省略或用省略号代替。
- 目标语言语法必须正确：名词的性与数要和冠词、形容词正确配合…（例如法语 haleine 为阴性，
  必须写 haleine fraîche，不能写 haleine frais）。
- 同一术语在整批中的译法必须完全一致。
要求：
1. 只翻译文本内容，保持原意、语气和专业术语；不要添加解释、不要输出多余内容。
2. 输出必须是 JSON 字符串数组，长度与输入完全一致，顺序一一对应。
输入：["Net wt. 120g","Keep out of reach of children"]
```

> 想知道**你当前设置**下模型到底收到什么？跑
> `node tools\show-prompt.mjs --preset=jiuye --to=法语 --demo=toothpaste`，
> 它会把你选的预设、目标语言、同版面上下文原样拼出提示词打印出来，可以直接贴到任意大模型里对比。

#### ⚠️ 只有大模型引擎支持语境

| 引擎 | 语境 | 说明 |
|---|---|---|
| **AI 大模型**（DeepSeek / **智谱 GLM-4-Flash（免费）** / 硅基流动 / Ollama / OpenAI） | ✅ 生效 | 语境描述 + 同版面上下文都注入提示词 |
| 腾讯 / Bing / 有道 / Google / MyMemory | ❌ 不生效 | 纯翻译接口，**没有接收指令的通道** |

面板右上角有状态徽标：显示 `已启用` 说明当前引擎吃语境；显示 `当前引擎不支持` 说明白填了（术语表仍然有效）。

点「翻译」时如果引擎不支持，日志里会明确提醒，不会让你以为设置了却没作用。

**想免费又要语境**：去 [智谱开放平台](https://open.bigmodel.cn/) 申请 API Key（`glm-4-flash` 有免费额度）→ 设置里引擎选「AI 大模型」→ 预设选「智谱 GLM-4-Flash」→ 填入 Key。或者装个 [Ollama](https://ollama.com/) 用本地模型，**完全离线且不要钱**（预设已内置 `http://127.0.0.1:11434/v1`）。

> 免费引擎也不是完全没办法：把行业术语填进**术语表**（3.4 节下方），所有引擎都会强制执行。这是零成本让免费引擎「懂行」的唯一办法。
...
- 打包后建议自检：`$env:TX_EXT_ROOT="<包内扩展目录>"; node tools\test-panel.mjs` 可以直接对包内代码跑 69 项 UI 断言。
...
# 真实引擎联网测试：62 项（= 上面 56 项离线 + 6 项真实调用引擎/代理探测；被限流的引擎记为 skip）
node tools\test-core.mjs --network
node tools\test-core.mjs --network --engine=bing      # 只测某个引擎

# 面板 UI 全流程测试：69 项（jsdom 真实加载 index.html，假宿主 + 假网络）
...
| 测试 | 覆盖内容 |
|---|---|
| `test-core.mjs` | 切分、语言识别、可译性过滤、去重、分批、术语表、译文清洗、大小写变换、LLM 输出解析、**语境提示词与解析**、结果合并 |
| `test-core.mjs --network` | 5 个免费引擎真实调用 + auto 降级链 + 代理自动探测 |
| `test-panel.mjs` | 面板启动、Node 网络层加载（含 `file:///` 路径回归）、引擎/语言下拉、扫描渲染、翻译（去重后只发 1 次请求）、**输出大写两条路径**、**语境注入与一键分析画布**、apply 载荷正确性、撤销、网络诊断 |
...
| 填了语境但感觉没生效 | 语境只有大模型引擎能接收。看「语境」标题右侧的徽标：显示`当前引擎不支持`就说明白填了 —— 请把引擎切到「AI 大模型」并填 Key（智谱 GLM-4-Flash 有免费额度），或改用术语表兜底 |
| 点「分析画布」报「语境分析需要大模型引擎」 | 同上：先在「高级设置」里选预设并填 API Key |
| 大模型报 401 / 403 | Key 错、过期，或 Base URL 与 Key 不是同一家。检查「高级设置」里的预设与自定义地址是否匹配 |
...
- **语境翻译只对大模型引擎生效**：腾讯 / Bing / 有道 / Google / MyMemory 是纯翻译接口，没有接收指令的通道；想免费用语境就用智谱 GLM-4-Flash（免费额度）或本地 Ollama
...
| 语境功能（v1.1） | 宿主脚本逻辑零改动，只改了 `VERSION` 常量 | 语境纯面板侧实现，只影响发给大模型的提示词 |
